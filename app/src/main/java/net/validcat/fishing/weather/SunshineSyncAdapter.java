...
//            setLocationStatus(getContext(), Constants.LOCATION_STATUS_SERVER_INVALID);
    ...
    public void getCurrentWeatherDataFromJson(String forecastJsonStr, IWeatherListener listener)
        ...
        try {
            ...
            JSONObject weatherObject = (JSONObject) forecastJson.getJSONArray(OWM_WEATHER).get(0);
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for(int i = 0; i < weatherArray.length(); i++) {
                ...
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_LOC_KEY, locationId);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_DATE, dateTime);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_HUMIDITY, humidity);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_PRESSURE, pressure);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_WIND_SPEED, windSpeed);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_DEGREES, windDirection);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_MAX_TEMP, high);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_MIN_TEMP, low);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_SHORT_DESC, description);
                weatherValues.put(FishingContract.WeatherEntry.COLUMN_WEATHER_ID, weatherId);
            ...
            if (cVVector.size() > 0) {
                ...
                getContext().getContentResolver().bulkInsert(FishingContract.WeatherEntry.CONTENT_URI, cvArray);

                // delete old data so we don't build up an endless history
                getContext().getContentResolver().delete(FishingContract.WeatherEntry.CONTENT_URI,
